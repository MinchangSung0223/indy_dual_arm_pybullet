...
class Kuka:
    def __init__(self, stepsize=1e-3, realtime=0):
        ...
        self.joint_init = [0,-0.2618,-1.5708,0,-1.309,0,0]

        # connect pybullet
        p.connect(p.GUI)
        p.configureDebugVisualizer(p.COV_ENABLE_GUI, 0)
        p.resetDebugVisualizerCamera(cameraDistance=5, cameraYaw=89, cameraPitch=-60, cameraTargetPosition=[0, 0, 0.5])
       ...
       # p.resetBasePositionAndOrientation(self.ball, self.default_ball_pos, self.default_ball_ori)
        ...
        # robot
        self.robot = p.loadURDF("kuka210/kr210l150.urdf",
                                useFixedBase=True,
                                flags=p.URDF_USE_SELF_COLLISION)
        self.robot_id = self.robot;
        # robot parameters
        self.dof = p.getNumJoints(self.robot)  # Virtual fixed joint between the flange and last link
       # print(self.dof)
        #if self.dof != 6:
